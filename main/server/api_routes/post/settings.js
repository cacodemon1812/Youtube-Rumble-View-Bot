...
async function updatePremiumRank() {
	...
	if (api_key) {
		try {
			let data = (
				await axios.get('https://www.bloxxy.net/api/patreon', {
					headers: {
			...
			if (data.id >= 0 && data.subscription >= 10) {
				...
				await dbRunWithValues(`INSERT OR REPLACE INTO premium_cache (value, date) VALUES (?, ?)`, [
					'true',
					Date.now()
				]);
			} else {
...
async function updateFreeRank() {
	...
	if (free_api_key) {
		try {
			let status = (
				await axios.get(`https://www.bloxxy.net/api/linkvertise/status/${free_api_key}`)
			).data;

			if (status) {
				hasFreeKey = true;

				await dbRunWithValues(`INSERT OR REPLACE INTO free_cache (value, date) VALUES (?, ?)`, [
					'true',
					Date.now()
				]);
			} else {
...
global.premiumText = `you must be a subscriber on our <a style="color: #3498db; text-decoration: none; cursor: pointer; background-color: none; font-size: 1em;" href="">Patreon</a>.
